...
> 数据截点：`2026-08-10T16:30:00+08:00`。状态由 `data/ledger/portfolio.json` 生成；所有金额均为虚拟记录。
...
|---|---|---:|---:|---:|---:|---:|
| 510300 | 华泰柏瑞沪深300ETF | 1200 | ¥4.6812 | ¥4.7590 | ¥5,710.80 | +93.40 |
| 510500 | 南方中证500ETF | 400 | ¥7.6653 | ¥8.0370 | ¥3,214.80 | +148.70 |
| 510880 | 华泰柏瑞上证红利ETF | 1100 | ¥3.2235 | ¥3.3190 | ¥3,650.90 | +105.00 |
| 512100 | 南方中证1000ETF | 900 | ¥2.8380 | ¥3.1290 | ¥2,816.10 | +261.90 |
| 588000 | 华夏上证科创板50成份ETF | 1200 | ¥1.8642 | ¥1.8350 | ¥2,202.00 | -35.00 |
...
2026-W32（8 月 3 日至 8 月 9 日）项目权益增加 **¥641.30（+2.148%）**。五只场内 ETF 贡献 ¥646.30 毛收益，扣除 ¥5.00 手续费后得到本周净收益；组合以 +2.148% 跑赢上证50的 +1.281%，但落后于上证指数、深证成指、创业板指和科创50。

本周仅 1/5 个交易日完成成交，8 月 6 日收盘报告缺失，基金任务也只有 2/5 次留下工件。五只持仓虽然没有重复 `exposure_group`，但宽基与成长 ETF 的 10 点短样本相关性较高，因此周报结论是“账本盈利、执行链存在缺口、标签分散不等于风险独立”，而不是用一周收益放宽策略。

完整证据、资产类别归因、天天基金核验、基准比较与风险诊断见 [`2026-W32 周报`](reports/weekly/2026-W32.md)；机器可读版本见 [`2026-W32.json`](reports/weekly/2026-W32.json)。

<!-- VIBE_DAILY_STRATEGY:START -->
## 当前市场判断

> 每日策略日期：**2026-08-10**；决策截点：`2026-08-10T16:30:00+08:00`；策略版本：`v0.3.1`。本区块由最新不可变报告自动生成，不再保留首日静态策略。

### 市场温度

行情观测截至 `2026-08-10T16:14:42+08:00`。
...
点时输入：[`2026-08-10.json`](data/inbox/2026-08-10.json)。

### 今日策略动作

| 代码 | 标的 | 动作 | 当前权重 | 主要依据 |
|---|---|---:|---:|---|
| 510300 | 华泰柏瑞沪深300ETF | 持有 | 18.7% | 历史仍处于冷启动；`SECURITY_IDENTITY_NOT_CLEARED`；`TERMINATION_RISK_NOT_CLEARED` |
| 588000 | 华夏上证科创板50成份ETF | 持有 | 7.2% | 历史仍处于冷启动；`TREND_NOT_CONFIRMED` |
| 510880 | 华泰柏瑞上证红利ETF | 观察 | 12.0% | 历史仍处于冷启动；冷启动趋势信号；`POSITION_TOO_SMALL_AFTER_DIVERSIFICATION` |
| 510500 | 南方中证500ETF | 观察 | 10.5% | 历史仍处于冷启动；冷启动趋势信号；`POSITION_TOO_SMALL_AFTER_DIVERSIFICATION` |
| 512100 | 南方中证1000ETF | 观察 | 9.2% | 历史仍处于冷启动；冷启动趋势信号；`POSITION_TOO_SMALL_AFTER_DIVERSIFICATION` |
| 159915 | 易方达创业板ETF | 观察 | 0.0% | 历史仍处于冷启动；`TREND_NOT_CONFIRMED` |
| 159928 | 汇添富中证主要消费ETF | 观察 | 0.0% | 历史仍处于冷启动；`TREND_NOT_CONFIRMED` |
| 510050 | 华夏上证50ETF | 观察 | 0.0% | 历史仍处于冷启动；`SECURITY_IDENTITY_NOT_CLEARED`；`TERMINATION_RISK_NOT_CLEARED` |
| 511010 | 国泰上证5年期国债ETF | 观察 | 0.0% | 历史仍处于冷启动；`SECURITY_IDENTITY_NOT_CLEARED`；`TERMINATION_RISK_NOT_CLEARED` |
| 511880 | 银华日利ETF | 观察 | 0.0% | 历史仍处于冷启动；`SECURITY_IDENTITY_NOT_CLEARED`；`TERMINATION_RISK_NOT_CLEARED` |

其余 3 个低优先级观察项保留在机器报告中。

决策报告：[`2026-08-10-short.json`](reports/daily/2026-08-10-short.json)。

### 今日执行与订单

- 开盘结算截点：`2026-08-10T09:34:29+08:00`；实际虚拟成交 **0** 笔，取消 **1** 笔。
- `CANCELLED_DATA_GATE` 买入 `510050` 1200 份；原因：`UNVERIFIED_SECURITY_IDENTITY_AND_ST_DELISTING`。
- 执行报告：[`2026-08-10-open.json`](reports/execution/2026-08-10-open.json)。
- 当前账本待执行订单：**0** 笔。

### 场外基金周期

- 最新净值周期：**2026-08-05** `2026-08-05T22:30:00+08:00`；扫描 7 只场外基金，可执行信号 0 个。
- 天天基金只作净值、申赎、费率等交叉检查；详情见 [`2026-08-05-funds.json`](reports/funds/2026-08-05-funds.json)。
...
> 这是依据 **2026-08-10** 最新证据生成的滚动计划；下一次 `update-readme` 会整体替换本区块，历史判断仍保留在不可变日报中。

| 阶段 | 计划 | 判定条件 |
|---|---|---|
| 本交易日后续 | 16:30 保存权益、黄金、国债和现金 ETF 的完整收盘截面并重新计算订单 | 数据类型齐全、双源一致、公司行为已核验 |
| 下一交易日 | 继续比较趋势与受控回撤信号，只在双源价格、交易状态和仓位上限全部通过时下单。 | 广泛冲击门、开盘时间窗、价格冲突与 T+1 |
