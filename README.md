...
> 数据截点：`2026-08-05T09:31:46+08:00`。状态由 `data/ledger/portfolio.json` 生成；所有金额均为虚拟记录。
...
| 510500 | 南方中证500ETF | 400 | ¥7.6653 | ¥7.6000 | ¥3,040.00 | -26.10 |
| 510880 | 华泰柏瑞上证红利ETF | 1100 | ¥3.2235 | ¥3.2280 | ¥3,550.80 | +4.90 |
| 512100 | 南方中证1000ETF | 900 | ¥2.8380 | ¥2.9300 | ¥2,637.00 | +82.80 |
| 588000 | 华夏上证科创板50成份ETF | 1200 | ¥1.8642 | ¥1.7060 | ¥2,047.20 | -189.80 |
...
> 每日策略日期：**2026-08-05**；决策截点：`2026-08-05T08:00:00+08:00`；策略版本：`v0.3.1`。本区块由最新不可变报告自动生成，不再保留首日静态策略。

### 市场温度

行情观测截至 `2026-08-04T16:14:51+08:00`。
...
点时输入：[`2026-08-05-preopen.json`](data/inbox/2026-08-05-preopen.json)。

### 今日策略动作

| 代码 | 标的 | 动作 | 当前权重 | 主要依据 |
|---|---|---:|---:|---|
| 512170 | 华宝中证医疗ETF | 买入 | 0.0% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 510880 | 华泰柏瑞上证红利ETF | 持有 | 12.0% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 512100 | 南方中证1000ETF | 持有 | 8.9% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 588000 | 华夏上证科创板50成份ETF | 持有 | 6.9% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 510300 | 华泰柏瑞沪深300ETF | 观察 | 18.8% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 510500 | 南方中证500ETF | 观察 | 10.2% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
...
决策报告：[`2026-08-05-preopen.json`](reports/preopen/2026-08-05-preopen.json)。

### 今日执行与订单

- 开盘结算截点：`2026-08-05T09:31:46+08:00`；实际虚拟成交 **0** 笔，取消 **1** 笔。
- `CANCELLED_DATA_GATE` 买入 `512170` 7100 份；原因：`UNVERIFIED_SECURITY_IDENTITY_AND_ST_DELISTING`。
- 执行报告：[`2026-08-05-open.json`](reports/execution/2026-08-05-open.json)。
- 当前账本待执行订单：**0** 笔。

### 场外基金周期

- 最新净值周期：**2026-08-04** `2026-08-04T22:30:00+08:00`；扫描 7 只场外基金，可执行信号 0 个。
- 天天基金只作净值、申赎、费率等交叉检查；详情见 [`2026-08-04-funds.json`](reports/funds/2026-08-04-funds.json)。
...
> 这是依据 **2026-08-05** 最新证据生成的滚动计划；下一次 `update-readme` 会整体替换本区块，历史判断仍保留在不可变日报中。
