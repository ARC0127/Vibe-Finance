...
> 数据截点：`2026-07-23T22:36:54+08:00`。状态由 `data/ledger/portfolio.json` 生成；所有金额均为虚拟记录。
...
> 每日策略日期：**2026-07-23**；决策截点：`2026-07-23T16:39:16+08:00`；策略版本：`v0.3.1`。本区块由最新不可变报告自动生成，不再保留首日静态策略。
...
点时输入：[`2026-07-23.json`](data/inbox/2026-07-23.json)。

### 今日策略动作

| 代码 | 标的 | 动作 | 当前权重 | 主要依据 |
|---|---|---:|---:|---|
| 510880 | 华泰柏瑞上证红利ETF | 买入 | 0.0% | 历史仍处于冷启动；冷启动趋势信号 |
| 512100 | 南方中证1000ETF | 加仓 | 7.7% | 历史仍处于冷启动；冷启动趋势信号 |
| 510300 | 华泰柏瑞沪深300ETF | 观察 | 19.1% | 历史仍处于冷启动；冷启动趋势信号；`POSITION_TOO_SMALL_AFTER_DIVERSIFICATION` |
| 159915 | 易方达创业板ETF | 观察 | 0.0% | 历史仍处于冷启动；冷启动趋势信号；`MAXIMUM_NEW_BUYS_REACHED` |
...
| 510500 | 南方中证500ETF | 观察 | 0.0% | 历史仍处于冷启动；`TREND_NOT_CONFIRMED` |
...
决策报告：[`2026-07-23-short.json`](reports/daily/2026-07-23-short.json)。
...
- 最新净值周期：**2026-07-23** `2026-07-23T22:36:54+08:00`；扫描 7 只场外基金，可执行信号 0 个。
- 天天基金只作净值、申赎、费率等交叉检查；详情见 [`2026-07-23-funds.json`](reports/funds/2026-07-23-funds.json)。
