...
> 数据截点：`2026-07-28T22:30:00+08:00`。状态由 `data/ledger/portfolio.json` 生成；所有金额均为虚拟记录。
...
|---|---|---:|---:|---:|---:|---:|
| 510300 | 华泰柏瑞沪深300ETF | 1200 | ¥4.6812 | ¥4.7530 | ¥5,703.60 | +86.20 |
| 510500 | 南方中证500ETF | 400 | ¥7.6653 | ¥7.7260 | ¥3,090.40 | +24.30 |
| 510880 | 华泰柏瑞上证红利ETF | 1100 | ¥3.2235 | ¥3.2320 | ¥3,555.20 | +9.30 |
| 512100 | 南方中证1000ETF | 800 | ¥2.8302 | ¥2.9300 | ¥2,344.00 | +79.80 |
| 588000 | 华夏上证科创板50成份ETF | 1200 | ¥1.8642 | ¥1.9110 | ¥2,293.20 | +56.20 |
...
> 每日策略日期：**2026-07-28**；决策截点：`2026-07-28T08:00:00+08:00`；策略版本：`v0.3.1`。本区块由最新不可变报告自动生成，不再保留首日静态策略。

### 市场温度

行情观测截至 `2026-07-27T16:14:21+08:00`。
...
点时输入：[`2026-07-28-preopen.json`](data/inbox/2026-07-28-preopen.json)。

### 今日策略动作

| 代码 | 标的 | 动作 | 当前权重 | 主要依据 |
|---|---|---:|---:|---|
| 588000 | 华夏上证科创板50成份ETF | 买入 | 0.0% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 159928 | 汇添富中证主要消费ETF | 买入 | 0.0% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 510880 | 华泰柏瑞上证红利ETF | 买入 | 0.0% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 510500 | 南方中证500ETF | 加仓 | 7.7% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 512100 | 南方中证1000ETF | 持有 | 7.8% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 510300 | 华泰柏瑞沪深300ETF | 观察 | 19.0% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 512480 | 国联安半导体ETF | 观察 | 0.0% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 159915 | 易方达创业板ETF | 观察 | 0.0% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 510050 | 华夏上证50ETF | 观察 | 0.0% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 511010 | 国泰上证5年期国债ETF | 观察 | 0.0% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |

其余 3 个低优先级观察项保留在机器报告中。

决策报告：[`2026-07-28-preopen.json`](reports/preopen/2026-07-28-preopen.json)。

### 今日执行与订单

- 开盘结算截点：`2026-07-28T09:31:44+08:00`；实际虚拟成交 **3** 笔，取消 **1** 笔。
- `FILLED` 买入 `510500` 100 份；原因：`COLD_START_TREND`。
- `FILLED` 买入 `588000` 1200 份；原因：`COLD_START_TREND`。
- `CANCELLED_DATA_GATE` 买入 `159928` 3300 份；原因：`UNVERIFIED_SECURITY_IDENTITY_AND_ST_DELISTING`。
- `FILLED` 买入 `510880` 1100 份；原因：`COLD_START_TREND`。
- 执行报告：[`2026-07-28-open.json`](reports/execution/2026-07-28-open.json)。
- 当前账本待执行订单：**0** 笔。

### 场外基金周期

- 最新净值周期：**2026-07-28** `2026-07-28T22:30:00+08:00`；扫描 7 只场外基金，可执行信号 0 个。
- 天天基金只作净值、申赎、费率等交叉检查；详情见 [`2026-07-28-funds.json`](reports/funds/2026-07-28-funds.json)。
...
> 这是依据 **2026-07-28** 最新证据生成的滚动计划；下一次 `update-readme` 会整体替换本区块，历史判断仍保留在不可变日报中。
