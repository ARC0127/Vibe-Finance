...
> 数据截点：`2026-08-03T09:33:00+08:00`。状态由 `data/ledger/portfolio.json` 生成；所有金额均为虚拟记录。
...
| 510500 | 南方中证500ETF | 400 | ¥7.6653 | ¥7.4610 | ¥2,984.40 | -81.70 |
| 510880 | 华泰柏瑞上证红利ETF | 1100 | ¥3.2235 | ¥3.2910 | ¥3,620.10 | +74.20 |
| 512100 | 南方中证1000ETF | 900 | ¥2.8380 | ¥2.8500 | ¥2,565.00 | +10.80 |
| 588000 | 华夏上证科创板50成份ETF | 1200 | ¥1.8642 | ¥1.6980 | ¥2,037.60 | -199.40 |
...
> 每日策略日期：**2026-08-03**；决策截点：`2026-08-03T08:00:00+08:00`；策略版本：`v0.3.1`。本区块由最新不可变报告自动生成，不再保留首日静态策略。

### 市场温度

行情观测截至 `2026-07-31T16:14:51+08:00`。
...
点时输入：[`2026-08-03-preopen.json`](data/inbox/2026-08-03-preopen.json)。

### 今日策略动作

| 代码 | 标的 | 动作 | 当前权重 | 主要依据 |
|---|---|---:|---:|---|
| 510050 | 华夏上证50ETF | 买入 | 0.0% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 512100 | 南方中证1000ETF | 加仓 | 7.7% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 510880 | 华泰柏瑞上证红利ETF | 持有 | 12.2% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 588000 | 华夏上证科创板50成份ETF | 持有 | 7.0% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 510300 | 华泰柏瑞沪深300ETF | 观察 | 18.8% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 510500 | 南方中证500ETF | 观察 | 10.1% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 512170 | 华宝中证医疗ETF | 观察 | 0.0% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 518880 | 华安黄金易ETF | 观察 | 0.0% | `COLD_START_GOLD_DIVERSIFIER`；历史仍处于冷启动；盘前交易状态待核验 |
...
决策报告：[`2026-08-03-preopen.json`](reports/preopen/2026-08-03-preopen.json)。

### 今日执行与订单

- 开盘结算截点：`2026-08-03T09:33:00+08:00`；实际虚拟成交 **1** 笔，取消 **1** 笔。
- `CANCELLED_DATA_GATE` 买入 `510050` 1300 份；原因：`UNVERIFIED_SECURITY_IDENTITY_AND_ST_DELISTING`。
- `FILLED` 买入 `512100` 100 份；原因：`COLD_START_TREND`。
- 执行报告：[`2026-08-03-open.json`](reports/execution/2026-08-03-open.json)。
- 当前账本待执行订单：**0** 笔。

### 场外基金周期

- 最新净值周期：**2026-07-31** `2026-07-31T22:30:00+08:00`；扫描 7 只场外基金，可执行信号 0 个。
- 天天基金只作净值、申赎、费率等交叉检查；详情见 [`2026-07-31-funds.json`](reports/funds/2026-07-31-funds.json)。
...
> 这是依据 **2026-08-03** 最新证据生成的滚动计划；下一次 `update-readme` 会整体替换本区块，历史判断仍保留在不可变日报中。
