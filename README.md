...
> 数据截点：`2026-08-17T13:05:10.284935+08:00`。状态由 `data/ledger/portfolio.json` 生成；所有金额均为虚拟记录。
...
|---|---|---:|---:|---:|---:|---:|
| 510300 | 华泰柏瑞沪深300ETF | 1200 | ¥4.6812 | ¥4.7720 | ¥5,726.40 | +109.00 |
| 510500 | 南方中证500ETF | 400 | ¥7.6653 | ¥8.1390 | ¥3,255.60 | +189.50 |
| 510880 | 华泰柏瑞上证红利ETF | 1300 | ¥3.2387 | ¥3.2970 | ¥4,286.10 | +75.80 |
| 512100 | 南方中证1000ETF | 900 | ¥2.8380 | ¥3.1950 | ¥2,875.50 | +321.30 |
| 518880 | 华安黄金易ETF | 300 | ¥9.0487 | ¥9.0570 | ¥2,717.10 | +2.50 |
| 588000 | 华夏上证科创板50成份ETF | 1300 | ¥1.8674 | ¥1.8690 | ¥2,429.70 | +2.10 |
...
> 每日策略日期：**2026-08-17**；决策截点：`2026-08-17T08:00:00+08:00`；策略版本：`v0.3.1`。本区块由最新不可变报告自动生成，不再保留首日静态策略。
...
点时输入：[`2026-08-17-preopen.json`](data/inbox/2026-08-17-preopen.json)。

### 今日策略动作

| 代码 | 标的 | 动作 | 当前权重 | 主要依据 |
|---|---|---:|---:|---|
| 510880 | 华泰柏瑞上证红利ETF | 加仓 | 11.9% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 510300 | 华泰柏瑞沪深300ETF | 持有 | 18.7% | 历史仍处于冷启动；盘前交易状态待核验；`SECURITY_IDENTITY_NOT_CLEARED` |
| 518880 | 华安黄金易ETF | 持有 | 8.8% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 588000 | 华夏上证科创板50成份ETF | 持有 | 7.8% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 159915 | 易方达创业板ETF | 观察 | 0.0% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
| 512100 | 南方中证1000ETF | 观察 | 9.3% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
...
决策报告：[`2026-08-17-preopen.json`](reports/preopen/2026-08-17-preopen.json)。

### 今日执行与订单

- 开盘结算截点：`2026-08-17T13:05:10.284935+08:00`；实际虚拟成交 **1** 笔，取消 **0** 笔。
- `FILLED` 买入 `510880` 200 份；原因：`DAILY_EXPLORATION_FALLBACK`。
- 执行报告：[`2026-08-17-intraday.json`](reports/execution/2026-08-17-intraday.json)。
- 当前账本待执行订单：**0** 笔。
...
> 这是依据 **2026-08-17** 最新证据生成的滚动计划；下一次 `update-readme` 会整体替换本区块，历史判断仍保留在不可变日报中。
