...
> 数据截点：`2026-08-04T09:30:11+08:00`。状态由 `data/ledger/portfolio.json` 生成；所有金额均为虚拟记录。
...
|---|---|---:|---:|---:|---:|---:|
| 510300 | 华泰柏瑞沪深300ETF | 1200 | ¥4.6812 | ¥4.6180 | ¥5,541.60 | -75.80 |
| 510500 | 南方中证500ETF | 400 | ¥7.6653 | ¥7.4700 | ¥2,988.00 | -78.10 |
| 510880 | 华泰柏瑞上证红利ETF | 1100 | ¥3.2235 | ¥3.2800 | ¥3,608.00 | +62.10 |
| 512100 | 南方中证1000ETF | 900 | ¥2.8380 | ¥2.8850 | ¥2,596.50 | +42.30 |
| 588000 | 华夏上证科创板50成份ETF | 1200 | ¥1.8642 | ¥1.6490 | ¥1,978.80 | -258.20 |
...
> 每日策略日期：**2026-08-04**；决策截点：`2026-08-04T08:00:00+08:00`；策略版本：`v0.3.1`。本区块由最新不可变报告自动生成，不再保留首日静态策略。

### 市场温度

行情观测截至 `2026-08-03T16:14:45+08:00`。
...
点时输入：[`2026-08-04-preopen.json`](data/inbox/2026-08-04-preopen.json)。

### 今日策略动作

| 代码 | 标的 | 动作 | 当前权重 | 主要依据 |
|---|---|---:|---:|---|
| 159915 | 易方达创业板ETF | 买入 | 0.0% | 历史仍处于冷启动；受控回撤信号；盘前交易状态待核验 |
| 510300 | 华泰柏瑞沪深300ETF | 持有 | 18.7% | 历史仍处于冷启动；受控回撤信号；盘前交易状态待核验 |
| 510500 | 南方中证500ETF | 持有 | 10.1% | 历史仍处于冷启动；受控回撤信号；盘前交易状态待核验 |
| 510880 | 华泰柏瑞上证红利ETF | 持有 | 12.2% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 512100 | 南方中证1000ETF | 持有 | 8.7% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 588000 | 华夏上证科创板50成份ETF | 持有 | 6.6% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
...
决策报告：[`2026-08-04-preopen.json`](reports/preopen/2026-08-04-preopen.json)。

### 今日执行与订单

- 开盘结算截点：`2026-08-04T09:30:11+08:00`；实际虚拟成交 **0** 笔，取消 **1** 笔。
- `CANCELLED_LIMIT_OR_CASH` 买入 `159915` 100 份；原因：`OPEN_ABOVE_LIMIT`。
- 执行报告：[`2026-08-04-open.json`](reports/execution/2026-08-04-open.json)。
- 当前账本待执行订单：**0** 笔。
...
> 这是依据 **2026-08-04** 最新证据生成的滚动计划；下一次 `update-readme` 会整体替换本区块，历史判断仍保留在不可变日报中。
