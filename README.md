...
> 数据截点：`2026-07-31T16:15:00+08:00`。状态由 `data/ledger/portfolio.json` 生成；所有金额均为虚拟记录。
...
| 510500 | 南方中证500ETF | 400 | ¥7.6653 | ¥7.5000 | ¥3,000.00 | -66.10 |
| 510880 | 华泰柏瑞上证红利ETF | 1100 | ¥3.2235 | ¥3.3040 | ¥3,634.40 | +88.50 |
| 512100 | 南方中证1000ETF | 800 | ¥2.8302 | ¥2.8640 | ¥2,291.20 | +27.00 |
| 588000 | 华夏上证科创板50成份ETF | 1200 | ¥1.8642 | ¥1.7280 | ¥2,073.60 | -163.40 |
...
> 每日策略日期：**2026-07-31**；决策截点：`2026-07-31T16:15:00+08:00`；策略版本：`v0.3.1`。本区块由最新不可变报告自动生成，不再保留首日静态策略。
...
点时输入：[`2026-07-31.json`](data/inbox/2026-07-31.json)。
...
| 512100 | 南方中证1000ETF | 加仓 | 7.7% | 历史仍处于冷启动；冷启动趋势信号 |
| 510880 | 华泰柏瑞上证红利ETF | 持有 | 12.2% | 历史仍处于冷启动；`TREND_NOT_CONFIRMED` |
| 588000 | 华夏上证科创板50成份ETF | 持有 | 7.0% | 历史仍处于冷启动；`TREND_NOT_CONFIRMED` |
| 510300 | 华泰柏瑞沪深300ETF | 观察 | 18.8% | 历史仍处于冷启动；冷启动趋势信号；`POSITION_TOO_SMALL_AFTER_DIVERSIFICATION` |
| 510500 | 南方中证500ETF | 观察 | 10.1% | 历史仍处于冷启动；冷启动趋势信号；`MAXIMUM_NEW_BUYS_REACHED` |
| 512170 | 华宝中证医疗ETF | 观察 | 0.0% | 历史仍处于冷启动；冷启动趋势信号；`MAXIMUM_NEW_BUYS_REACHED` |
| 518880 | 华安黄金易ETF | 观察 | 0.0% | `COLD_START_GOLD_DIVERSIFIER`；历史仍处于冷启动；`MAXIMUM_NEW_BUYS_REACHED` |
...
决策报告：[`2026-07-31-short.json`](reports/daily/2026-07-31-short.json)。

### 今日执行与订单

- 开盘结算截点：`2026-07-31T09:32:41+08:00`；实际虚拟成交 **0** 笔，取消 **1** 笔。
- `CANCELLED_LIMIT_OR_CASH` 买入 `159928` 3300 份；原因：`OPEN_ABOVE_LIMIT`。
- 执行报告：[`2026-07-31-open.json`](reports/execution/2026-07-31-open.json)。
- 当前账本待执行订单：**2** 笔。

### 场外基金周期

- 最新净值周期：**2026-07-30** `2026-07-30T22:30:00+08:00`；扫描 7 只场外基金，可执行信号 0 个。
- 天天基金只作净值、申赎、费率等交叉检查；详情见 [`2026-07-30-funds.json`](reports/funds/2026-07-30-funds.json)。
...
> 这是依据 **2026-07-31** 最新证据生成的滚动计划；下一次 `update-readme` 会整体替换本区块，历史判断仍保留在不可变日报中。
