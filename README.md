...
> [!WARNING]
> **2026-09-18 盘前审计：`FAILED_PREOPEN_RISK_REVIEW`。** `ORDER_SCHEDULED` 仅表示保留一笔黄金 ETF 条件单，不是成交许可。9 月 17 日封存收盘缺失，价格仍来自 9 月 16 日。按封存价和最低费用投影，小盘成长桶为 20.22454%，超过配置的 20% 上限。结算路径尚缺执行期资产桶上限重检，生产代码修复不在本盘前任务的发布合同内。未完成新鲜价格和完整风控证明前不得虚拟成交。见[独立风险审计](reports/research/2026-09-18-preopen-review-audit.md)。
...
> 数据截点：`2026-09-18T09:30:33+08:00`。状态由 `data/ledger/portfolio.json` 生成；所有金额均为虚拟记录。
...
|---|---|---:|---:|---:|---:|---:|
| 159928 | 汇添富中证主要消费ETF | 3500 | ¥0.6632 | ¥0.6480 | ¥2,268.00 | -53.12 |
| 510300 | 华泰柏瑞沪深300ETF | 1200 | ¥4.6812 | ¥4.5560 | ¥5,467.20 | -150.20 |
| 510500 | 南方中证500ETF | 300 | ¥7.7072 | ¥7.7460 | ¥2,323.80 | +11.65 |
| 510880 | 华泰柏瑞上证红利ETF | 1300 | ¥3.2540 | ¥3.3330 | ¥4,332.90 | +102.67 |
| 512100 | 南方中证1000ETF | 1200 | ¥2.9111 | ¥3.0910 | ¥3,709.20 | +215.90 |
| 518880 | 华安黄金易ETF | 300 | ¥9.0274 | ¥8.9500 | ¥2,685.00 | -23.21 |
...
2026-W37（9 月 7 日至 9 月 13 日）周末项目权益为 **¥29,886.20**，相对初始 ¥30,000 累计亏损 **¥113.80（-0.379%）**。9 月 7 日完整收盘基线缺失，因此本周正式收益率、波动和最大回撤不计算；从该日不完整账本快照到 9 月 11 日封存收盘的可核对净变动为 **-¥372.70**，其中费用 ¥20.00。

本周 4/5 个交易日完成虚拟成交；9 月 7 日因盘前与开盘流水线缺失没有成交。权益 ETF 的可核对区间净贡献为 -¥288.60，黄金 ETF 为 -¥84.10。当前 6 个持仓达到数量上限；中小盘成长桶占 18.796%，红利桶占 14.922%，各风险桶均未越线。基金任务覆盖 5/5；天天基金页面与 token 可达，但没有与基金公司同日净值形成可接受配对，因此场外基金保持零交易。

独立反思门禁仍只有 1 个完整往返、0 个 eligible sample，v0.4.0 本周不改参数。报告同时公开 10 个标准阶段文件缺口、W36 周报缺口和本次延迟触发事实；完整证据、资产类别归因、天天基金核验、基准比较与风险诊断见 [`2026-W37 周报`](reports/weekly/2026-W37.md)，机器可读版本见 [`2026-W37.json`](reports/weekly/2026-W37.json)。

<!-- VIBE_DAILY_STRATEGY:START -->
## 当前市场判断

> 每日策略日期：**2026-09-18**；决策截点：`2026-09-18T08:00:00+08:00`；策略版本：`v0.4.0`。本区块由最新不可变报告自动生成，不再保留首日静态策略。

### 市场温度

行情观测截至 `2026-09-16T16:14:42+08:00`。
...
点时输入：[`2026-09-18-preopen.json`](data/inbox/2026-09-18-preopen.json)。

### 今日策略动作

| 代码 | 标的 | 动作 | 当前权重 | 主要依据 |
|---|---|---:|---:|---|
| 518880 | 华安黄金易ETF | 加仓 | 6.0% | `COLD_START_GOLD_DIVERSIFIER`；盘前交易状态待核验 |
| 512100 | 南方中证1000ETF | 持有 | 12.4% | 冷启动趋势信号；盘前交易状态待核验 |
| 159928 | 汇添富中证主要消费ETF | 持有 | 7.6% | 盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 510300 | 华泰柏瑞沪深300ETF | 持有 | 18.4% | 盘前交易状态待核验；`SECURITY_IDENTITY_NOT_CLEARED`；`TERMINATION_RISK_NOT_CLEARED` |
| 510880 | 华泰柏瑞上证红利ETF | 持有 | 14.7% | 盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 159915 | 易方达创业板ETF | 观察 | 0.0% | 冷启动趋势信号；盘前交易状态待核验；`MAXIMUM_POSITIONS_REACHED` |
...
决策报告：[`2026-09-18-preopen.json`](reports/preopen/2026-09-18-preopen.json)。

### 今日执行与订单

- 开盘结算截点：`2026-09-18T09:30:33+08:00`；实际虚拟成交 **1** 笔，取消 **0** 笔。
- `FILLED` 买入 `518880` 100 份；原因：`DAILY_EXPLORATION_FALLBACK`。
- 执行报告：[`2026-09-18-open.json`](reports/execution/2026-09-18-open.json)。
- 当前账本待执行订单：**0** 笔。

### 场外基金周期

- 最新净值周期：**2026-09-17** `2026-09-17T22:30:00+08:00`；扫描 7 只场外基金，可执行信号 0 个。
- 天天基金只作净值、申赎、费率等交叉检查；详情见 [`2026-09-17-funds.json`](reports/funds/2026-09-17-funds.json)。
...
> 这是依据 **2026-09-18** 最新证据生成的滚动计划；下一次 `update-readme` 会整体替换本区块，历史判断仍保留在不可变日报中。
