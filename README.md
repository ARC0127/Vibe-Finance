...
> 数据截点：`2026-07-24T22:30:00+08:00`。状态由 `data/ledger/portfolio.json` 生成；所有金额均为虚拟记录。
...
|---|---|---:|---:|---:|---:|---:|
| 510300 | 华泰柏瑞沪深300ETF | 1200 | ¥4.6812 | ¥4.7870 | ¥5,744.40 | +127.00 |
| 512100 | 南方中证1000ETF | 800 | ¥2.8302 | ¥2.9140 | ¥2,331.20 | +67.00 |
...
> 每日策略日期：**2026-07-24**；决策截点：`2026-07-24T08:00:00+08:00`；策略版本：`v0.3.1`。本区块由最新不可变报告自动生成，不再保留首日静态策略。

### 市场温度

行情观测截至 `2026-07-23T16:39:16+08:00`。
...
点时输入：[`2026-07-24-preopen.json`](data/inbox/2026-07-24-preopen.json)。
...
| 159915 | 易方达创业板ETF | 买入 | 0.0% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
...
| 510300 | 华泰柏瑞沪深300ETF | 观察 | 19.1% | 历史仍处于冷启动；冷启动趋势信号；盘前交易状态待核验 |
...
| 510500 | 南方中证500ETF | 观察 | 0.0% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 511010 | 国泰上证5年期国债ETF | 观察 | 0.0% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |
| 511880 | 银华日利ETF | 观察 | 0.0% | 历史仍处于冷启动；盘前交易状态待核验；`TREND_NOT_CONFIRMED` |

其余 3 个低优先级观察项保留在机器报告中。

决策报告：[`2026-07-24-preopen.json`](reports/preopen/2026-07-24-preopen.json)。
...
> 这是依据 **2026-07-24** 最新证据生成的滚动计划；下一次 `update-readme` 会整体替换本区块，历史判断仍保留在不可变日报中。
