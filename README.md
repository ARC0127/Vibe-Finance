...
> 数据截点：`2026-09-16T22:30:00+08:00`。状态由 `data/ledger/portfolio.json` 生成；所有金额均为虚拟记录。
...
> 每日策略日期：**2026-09-16**；决策截点：`2026-09-16T16:30:00+08:00`；策略版本：`v0.4.0`。本区块由最新不可变报告自动生成，不再保留首日静态策略。
...
点时输入：[`2026-09-16.json`](data/inbox/2026-09-16.json)。

### 今日策略动作

| 代码 | 标的 | 动作 | 当前权重 | 主要依据 |
|---|---|---:|---:|---|
| 518880 | 华安黄金易ETF | 加仓 | 6.0% | `COLD_START_GOLD_DIVERSIFIER`；`POSITION_TOO_SMALL_AFTER_DIVERSIFICATION`；每日小额探索 |
| 512100 | 南方中证1000ETF | 持有 | 12.4% | 冷启动趋势信号 |
| 159928 | 汇添富中证主要消费ETF | 持有 | 7.6% | `TREND_NOT_CONFIRMED` |
| 510300 | 华泰柏瑞沪深300ETF | 持有 | 18.4% | `SECURITY_IDENTITY_NOT_CLEARED`；`TERMINATION_RISK_NOT_CLEARED` |
| 510880 | 华泰柏瑞上证红利ETF | 持有 | 14.7% | `TREND_NOT_CONFIRMED` |
...
决策报告：[`2026-09-16-short.json`](reports/daily/2026-09-16-short.json)。
