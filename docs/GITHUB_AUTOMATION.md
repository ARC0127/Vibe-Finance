...
- 同步分支：用户已明确授权由本项目独占维护仓库；9 个定时直接提交并推送 `main`。
...
以下条件现已完成：安装并认证 GitHub CLI、确认仓库可见性为 PUBLIC/PRIVATE 且权限为 ADMIN、验证非交互 Git，以及用户对直接维护 `main` 的明确授权。所有运行仍必须：
...
脚本支持九个固定 task-id，使用 `$HOME/.cache/vibe-finance/github-sync.lock` 跨任务互斥，并在每次提交前从账本刷新 README 状态、生成 `reports/automation-runs/<task-id>/` 机器审计清单。脚本不允许 force push、自动 rebase、删除或从非 `main` 分支同步。
